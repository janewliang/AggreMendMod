...
prob_df = sapply(1:length(c("10", "30", "50")), function(j) {
  lapply(dimnames(results[[1]]$probs[[1]])[[1]], function(i){
    data.frame(t(sapply(results, function(x){
      x$probs[[j]][i,]
    })))
  })
})
names(prob_df) = paste0(rep(c("PP22_All", "PP22_First", "Agg"), 3), "_", 
                        rep(c(10, 30, 50), each = 3))
