...
reshaped_boot_diagnostics = lapply(1:length(all_diagnostics_boot[[1]]),
                                   function(i){
                                     abind(lapply(all_diagnostics_boot, 
                                                  function(x){ x[[i]] }), along=3)
                                   })
...
names(boot_diagnostics_summary) = names(all_diagnostics_boot[[1]])
